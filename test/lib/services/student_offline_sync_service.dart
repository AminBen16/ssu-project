...
/// Service for managing offline sync operations specific to student data
class StudentOfflineSyncService {
  ...
  /// Performs bulk sync for student data (alias for syncParentData)
  Future<void> performBulkSync(
      {required String userId, required String schoolId}) async {
    await syncParentData(schoolId: schoolId, parentId: userId);
  }

  /// Gets pending operations from the offline service
  Future<List<Map<String, dynamic>>> getPendingOperations() async {
    try {
      final status = await _offlineService.getOfflineDataStatus();
      // Assuming the offline service returns a map with pending operations
      // This is a placeholder - adjust based on actual offline service implementation
      return status['pendingOperations'] as List<Map<String, dynamic>>? ?? [];
    } catch (e) {
  ...
  /// Performs immediate sync of all pending operations
  Future<void> syncNow() async {
    try {
      // Get pending operations and sync them
      final pendingOps = await getPendingOperations();
      if (pendingOps.isNotEmpty) {
        // Perform sync - this is a placeholder implementation
        // In a real implementation, you'd process each pending operation
        // For now, just perform the bulk sync
        // await _offlineService.syncPendingOperations();
      }
    } catch (e) {
      _logger.e('Error during syncNow: $e');
      rethrow;
    }
  }
}
