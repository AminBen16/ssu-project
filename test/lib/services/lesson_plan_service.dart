...
class LessonPlanService {
  ...
  final LessonPlanTemplateService _templateService = LessonPlanTemplateService();
  ...
  Future<LessonPlan> generateFromTemplate({
    ...
    try {
      ...
      final lessonPlan = LessonPlan(
        ...
        date: date,
        startTime: startTime,
        endTime: startTime.add(Duration(minutes: _parseDuration(template.duration))),
        duration: template.duration,
        learningObjectives: learningObjectives,
        teachingMethods: [template.teachingStyle],
        activities: template.activities,
        assessments: [template.assessment],
        materials: _mergeMaterials(template.materials, availableResources ?? []),
  ...
  List<LessonPlanTemplate> getAvailableTemplates({String? subject, String? category}) {
    if (subject != null) {
  ...
  List<String> _mergeMaterials(List<String> templateMaterials, List<String> availableResources) {
  ...
  String _generateTemplateNotes(LessonPlanTemplate template, Map<String, dynamic> curriculumData, Map<String, dynamic>? customNotes) {
    final notes = <String>[];
  ...
  Future<LessonPlan> generateAutoLessonPlan({
    ...
    try {
      ...
      final assessments = _generateAssessments(learningObjectives, lessonStructure);
      
      // Create teaching materials list
      final materials = _generateMaterials(curriculumData, availableResources ?? []);
      ...
      final lessonPlan = LessonPlan(
        ...
        endTime: DateTime.now().add(Duration(minutes: _parseDuration(duration))),
        duration: duration,
        learningObjectives: learningObjectives,
  ...
  Future<Map<String, dynamic>> _getCurriculumData(String topicId) async {
    try {
      ...
      final activities = await _curriculumService.getActivitiesByOutcome(int.parse(topicId));
      
      // Validate learning outcomes exist
      if (learningOutcomes.isEmpty) {
        throw Exception('No learning outcomes found for this topic in NCDC curriculum');
      }

      // Validate activities exist
      if (activities.isEmpty) {
        // Log warning but don't fail - some topics may not have predefined activities
        print('Warning: No predefined activities found for topic ${topic.name}');
      }
      ...
      print('Error fetching curriculum data for topic $topicId: $e');
      
      // Return structured fallback data instead of random data
      return {
        'topic': 'NCDC Curriculum Topic',
        'competency': 'Students understand the topic according to NCDC standards',
  ...
  Future<List<Map<String, dynamic>>> _generateLearningActivities(
    ...
    activities.add({
      ...
      'instructions': _generateIntroductionInstructions(curriculumData['topic']),
    });
    ...
    if (style.contains('interactive') || style.contains('collaborative')) {
      activities.addAll(_generateInteractiveActivities(curriculumData, learningObjectives, availableResources, lessonStructure));
    } else if (style.contains('lecture') || style.contains('direct')) {
      activities.addAll(_generateDirectInstructionActivities(curriculumData, learningObjectives, availableResources, lessonStructure));
    } else {
      activities.addAll(_generateMixedActivities(curriculumData, learningObjectives, availableResources, lessonStructure));
    }
    ...
    return [
      {
        ...
        'description': 'Students work in groups to investigate ${curriculumData['topic']}',
        'duration': lessonStructure['mainActivity1'] ?? 30,
        'type': 'group_work',
        'resources': ['Worksheet', 'Research materials'],
        'instructions': 'Divide students into groups of 4-5. Each group investigates different aspects of the topic and presents findings.',
      },
      if (lessonStructure.containsKey('mainActivity2'))
      {
        ...
        'type': 'practical',
        'resources': availableResources.isNotEmpty ? availableResources : ['Activity materials'],
        'instructions': 'Students engage in hands-on activities to reinforce learning',
      }
    ];
  }
    ...
    return [
      {
        ...
        'description': 'Teacher-led presentation of ${curriculumData['topic']} concepts',
        'duration': lessonStructure['mainActivity1'] ?? 30,
        'type': 'lecture',
        'resources': ['Presentation slides', 'Whiteboard'],
        'instructions': 'Present key concepts with clear explanations and examples',
      },
      if (lessonStructure.containsKey('mainActivity2'))
      {
    ...
    return [
      {
        ...
        'instructions': 'Present content with frequent student questions and participation',
      },
      if (lessonStructure.containsKey('mainActivity2'))
      {
        ...
        'instructions': 'Students work in pairs to complete practice exercises',
      }
    ];
  }

  /// Generate assessment methods
  List<Map<String, dynamic>> _generateAssessments(List<String> learningObjectives, Map<String, dynamic> lessonStructure) {
    return [
      {
        'type': 'formative',
        'name': 'Quick Quiz',
        ...
        'duration': 5,
        'questions': learningObjectives.map((obj) => {
  ...
  List<String> _generateMaterials(Map<String, dynamic> curriculumData, List<String> availableResources) {
    final materials = <String>[];
    
    // Add standard materials
    materials.addAll(['Whiteboard/Smartboard', 'Marker pens', 'Lesson plan handout']);
  ...
  /// Generate homework assignment
  Map<String, dynamic> _generateHomework(Map<String, dynamic> curriculumData, List<String> learningObjectives) {
    return {
      'assigned': true,
      'description': 'Complete practice exercises on ${curriculumData['topic']}',
      'tasks': learningObjectives.map((obj) => 'Practice problems related to: $obj').toList(),
  ...
  List<String> _generateTeachingMethods(String? preferredTeachingStyle) {
    ...
    if (style.contains('interactive')) {
      ...
      return ['Mixed Methods', 'Interactive Lecture', 'Collaborative Learning', 'Practice Activities'];
    }
  }
  ...
  String _generateLessonNotes(Map<String, dynamic> curriculumData, int? studentCount) {
    final notes = <String>[];
  ...
  Future<void> saveLessonPlan(LessonPlan lessonPlan) async {
    ...
    if (isOnline) {
      try {
        await _apiClient.post('/api/schools/${lessonPlan.schoolId}/lesson-plans',
  ...
  Future<Map<String, dynamic>> generateDynamicExam({
    ...
    required List<String> questionTypes, // ['multiple_choice', 'short_answer', 'essay']
    required int durationMinutes,
  }) async {
    try {
      // Validate question types against NCDC guidelines
      final typesValid = await _aiValidationService.validateQuestionTypes(questionTypes);
      if (!typesValid) {
        throw Exception('Some question types are not approved by NCDC guidelines');
      }

      // Validate curriculum data exists
      final validation = await _validateCurriculumData(topicId);
      if (!validation['isValid']) {
        throw Exception('Topic not found in NCDC curriculum or insufficient data');
      }

      final curriculumData = validation['curriculumData'] as Map<String, dynamic>;
      ...
      final validationResult = await _aiValidationService.validateAIGeneratedContent(
        contentType: 'exam',
        topicId: topicId,
      ...
      if (!validationResult['isValid']) {
        throw Exception('Generated exam failed validation: ${validationResult['errors']}');
      }
  ...
  Future<Map<String, dynamic>> _validateCurriculumData(String topicId) async {
    try {
      ...
      return {'isValid': false, 'error': 'Failed to validate curriculum data: $e'};
    }
  }
  ...
  Future<bool> validateQuestionTypes(List<String> questionTypes) async {
    final approvedTypes = ['multiple_choice', 'short_answer', 'essay', 'true_false', 'matching'];
  ...
  Map<String, dynamic> _generateMultipleChoiceQuestion(String topic, String outcome, int index) {
    final options = [
    ...
    return {
      ...
      'question': 'Based on the learning outcome: $outcome, which of the following best describes the key concept in $topic?',
      'options': options,
      'correctAnswer': options[0], // In real implementation, this would be intelligently generated
      'marks': 20,
      'outcome': outcome,
    };
  }

  Map<String, dynamic> _generateShortAnswerQuestion(String topic, String outcome, int index) {
    return {
      'id': 'q_${index + 1}',
      'type': 'short_answer',
      'question': 'Based on the learning outcome: $outcome, briefly explain the importance of $topic in your own words.',
      'expectedAnswer': 'Student should demonstrate understanding of $topic and its relevance to $outcome',
      'marks': 20,
      'outcome': outcome,
    };
  }

  Map<String, dynamic> _generateEssayQuestion(String topic, String outcome, int index) {
    return {
      'id': 'q_${index + 1}',
      'type': 'essay',
      'question': 'Based on the learning outcome: $outcome, write a comprehensive essay discussing the principles and applications of $topic.',
      'guidelines': [
  ...
  String _generateExamInstructions(List<String> questionTypes) {
    ...
    if (questionTypes.contains('multiple_choice')) {
      instructions.add('For multiple choice questions, select the best answer.');
    }
    if (questionTypes.contains('short_answer')) {
      instructions.add('For short answer questions, provide concise but complete responses.');
    }
    if (questionTypes.contains('essay')) {
      instructions.add('For essay questions, organize your thoughts and write clearly.');
    }
    
    return instructions.join('\n');
  }

  Map<String, dynamic> _generateTrueFalseQuestion(String topic, String outcome, int index) {
    return {
      'id': 'q_${index + 1}',
      'type': 'true_false',
      'question': 'Based on the learning outcome: $outcome, the following statement about $topic is true or false: $topic is fundamental to understanding this concept.',
      'correctAnswer': true, // In real implementation, this would be intelligently generated
      'marks': 20,
      'outcome': outcome,
    };
  }

  Map<String, dynamic> _generateMatchingQuestion(String topic, String outcome, int index) {
    return {
      'id': 'q_${index + 1}',
      'type': 'matching',
      'question': 'Based on the learning outcome: $outcome, match the following terms related to $topic with their definitions.',
      'items': [
