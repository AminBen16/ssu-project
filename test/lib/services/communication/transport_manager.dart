...
class RealTransportManager implements TransportManager {
  ...
  Future<void> start() async {
    ...
    try {
      developer.log(
          'Starting real transport manager with Bluetooth, Wi-Fi Direct, LoRa, and Satellite...');
      ...
      for (final transport in _transports) {
        try {
          if (await transport.isAvailable()) {
            ...
            developer.log('Started transport: ${transport.id}');
          } else {
            developer
                .log('Transport ${transport.id} not available on this device');
          }
        } catch (e) {
          developer.log('Failed to start transport ${transport.id}: $e');
        }
      }

      _isStarted = true;
      developer.log('Real transport manager started successfully');
    } catch (e) {
      developer.log('Failed to start transport manager: $e');
      rethrow;
    }
  }
  ...
  Future<void> stop() async {
    ...
    try {
      developer.log('Stopping real transport manager...');

      // Stop all transport layers
      for (final transport in _transports) {
        try {
          await transport.stop();
          developer.log('Stopped transport: ${transport.id}');
        } catch (e) {
          developer.log('Failed to stop transport ${transport.id}: $e');
        }
      }

      _isStarted = false;
      developer.log('Real transport manager stopped');
    } catch (e) {
      developer.log('Failed to stop transport manager: $e');
    }
  }
  ...
  Future<void> sendData(String data) async {
    ...
    for (final transport in sortedTransports) {
      try {
        if (await transport.isAvailable()) {
          developer.log('Attempting to send data via ${transport.id}');

          // Actually send the data using the transport
          await transport.sendData(data, ''); // Empty string for broadcast
          sent = true;
          developer.log('Successfully sent data via ${transport.id}');
          break;
        }
      } catch (e) {
        lastError = e.toString();
        developer.log('Failed to send via ${transport.id}: $e');
      }
    }

    if (!sent) {
      throw Exception(
          'Failed to send data via any transport. Last error: $lastError');
    }
  }
      ...
      try {
        if (transport is BluetoothTransport) {
          ...
          devices[transport.id] = [
            {'type': 'lora_range', 'status': 'listening'}
          ];
          ...
          devices[transport.id] = [
            {'type': 'satellite_network', 'status': status}
          ];
        }
      } catch (e) {
        developer.log('Failed to get devices from ${transport.id}: $e');
        devices[transport.id] = [];
      }
    }
    return devices;
  ...
  Future<bool> sendDataToDevice(String data, String deviceId,
      {String? transportId}) async {
    if (!_isStarted) {
      throw Exception('Transport manager not started');
    }
    ...
    if (targetTransport == null) {
      developer.log('No available transport found for device $deviceId');
      return false;
    }
    ...
    try {
      ...
      developer.log(
          'Failed to send data to $deviceId via ${targetTransport.id}: $e');
      return false;
    }
  }
